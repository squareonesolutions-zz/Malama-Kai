...
function mooring_admin_scripts() { 
	...
	wp_register_style("mrng", 'http://malama-kai.org/blog/wp-content/plugins/mooring-manager/style.css');
...
function moorings() { $moorings = get_moorings();?>
	...
	<?php foreach($moorings as $item) : $row = ($row == 'even') ? 'odd' : 'even'; ?>
    	<div id="mooring-<?php echo $item['ID']; ?>" class="mooring <?php echo $row; ?>">
	        ...
	        <tr>
	            <td valign="top"><span class="mooring-label">Mooring Depth: </span></td><td valign="top"><span class="mooring-info"><?php if(empty($item['bottom-depth'])) { echo '-'; } else { echo $item['bottom-depth']; } ?></span>
